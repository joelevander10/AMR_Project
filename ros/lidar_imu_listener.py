...
def imu_callback(data, sensor_data):
    sensor_data.acceleration = (data.linear_acceleration.x, data.linear_acceleration.y, d>
    sensor_data.gyroscope = (data.angular_velocity.x, data.angular_velocity.y, data.angul>

def main():
    rospy.init_node('lidar_imu_listener', anonymous=True)
    sensor_data = SensorData()

    lidar_sub = rospy.Subscriber('/lslidar_point_cloud', PointCloud2, lidar_callback, sen>
    imu_sub = rospy.Subscriber('imu/data_raw', Imu, imu_callback, sensor_data)
    twist_pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)

    rate = rospy.Rate(22)  # Lakukan pemrosesan data dengan frekuensi 10 Hz
    ...
    while not rospy.is_shutdown():
        ...
        if acceleration is not None and prev_velocity is not None:
            imu_velocity = tuple((prev_vel + accel) * dt / 2 for prev_vel, accel in zip(p>
        else:
            imu_velocity = (0.0, 0.0, 0.0)  # Set default velocity if data is not availab>
        ...
        rospy.loginfo("IMU Velocity: X: {:.3f} Y: {:.3f} Z: {:.3f} m/s".format(imu_veloci>
